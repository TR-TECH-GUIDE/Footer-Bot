...
@SLBotsOfficial.on_message(filters.private & filters.command("start"))
    ...
    try:
        await m.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [InlineKeyboardButton("Support Group", url="https://t.me/trtechguide"), InlineKeyboardButton("Bots Channel", url="https://t.me/SLBotsOfficial")],
                    [InlineKeyboardButton("Developer - @TharukRenuja", url="https://t.me/TharukRenuja")]
                ]
            ),
            quote=True
        )
...
@SLBotsOfficial.on_message(filters.private & filters.command("settings"))
...
@SLBotsOfficial.on_message(filters.channel & (filters.video | filters.document) & ~filters.edited & ~filters.private)
...
@SLBotsOfficial.on_message(filters.channel & filters.text & ~filters.edited & ~filters.private, group=-1)
...
@SLBotsOfficial.on_message(filters.channel & filters.photo & ~filters.edited & ~filters.private)
async def add_text_footer(bot: Client, event: Message):
    on_event = await db.find_user_id(event.chat.id)
    if on_event is None:
        return
...
@SLBotsOfficial.on_message(filters.private & filters.command("broadcast") & filters.user(Config.BOT_OWNER) & filters.reply)
async def _broadcast(_, event: Message):
    await broadcast_handler(event)


@SLBotsOfficial.on_message(filters.private & filters.command("status") & filters.user(Config.BOT_OWNER))
...
@SLBotsOfficial.on_message(filters.private & filters.command("disable") & filters.user(Config.BOT_OWNER))
        ...
        try:
            ...
            if event_.forward_from_chat and ((await db.is_user_exist(event_.forward_from_chat.id)) is False):
                try:
                    ...
                    await cb.message.edit(f"Unable to Find Channel!\n\n**Error:** `{err}`", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Support Group", url="https://t.me/trtechguide")]]))
